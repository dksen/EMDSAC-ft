__all__ = ["ApproxContainer", "EMDSAC"]
...
class EMDSAC:
    ...
    def __compute_gradient(self, data: Dict, iteration: int):
        ...
        #for name,param in self.networks.policy.named_parameters():
    ...
    def __compute_loss_policy(self, data: Dict):
        ...
        if self.it%5000==0:
            print(torch.mean(torch.stack(q_values)))
    ...
    def cal_ur(self, obs, act,obs2):
        ...
        q_means, q_stds = [], []
        q_ood_means,q_random_means=[],[]
        q_next_means, q_next_samples = [], []
        q_ood_stds,q_random_stds=[],[]


        for q in self.networks.q_networks:
            q_mean, q_std, _ = self.__q_evaluate(obs, act, q)
            q_ood_mean,q_ood_std, _= self.__q_evaluate(obs2, act2, q)
            q_random_mean,q_random_std,_=self.__q_evaluate(obs2, random_act, q)
            q_means.append(q_mean)
            q_ood_means.append(q_ood_mean)
            q_random_means.append(q_random_mean)
            q_stds.append(q_std)
            q_ood_stds.append(q_ood_std)
            q_random_stds.append(q_random_std)

        q_means_tensor=torch.stack(q_means)#[4, 256]
        q_ood_means_tensor = torch.stack(q_ood_means)
        q_random_means_tensor=torch.stack(q_random_means)

        q_stds_tensor=torch.stack(q_stds)#[4, 256]
        q_ood_stds_tensor = torch.stack(q_ood_stds)
        q_random_stds_tensor=torch.stack(q_random_stds)

        uncertainty_in=torch.mean(q_means_tensor, dim=0)-torch.min(q_means_tensor,dim=0).values
        uncertainty_ood = torch.mean(q_ood_means_tensor, dim=0) - torch.min(q_ood_means_tensor, dim=0).values
        uncertainty_random = torch.mean(q_random_means_tensor, dim=0) - torch.min(q_random_means_tensor, dim=0).values

        randomness_in=torch.mean(q_stds_tensor, dim=0)
        randomness_ood = torch.mean(q_ood_stds_tensor, dim=0)
        randomness_random = torch.mean(q_random_stds_tensor, dim=0)
