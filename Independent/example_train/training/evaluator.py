...
class Evaluator:
    ...
    def run_n_episodes_withstd(self, n, iteration):
        episode_return_list = []
        for _ in range(n):
            episode_return_list.append(self.run_an_episode(iteration, self.render))
        return np.mean(episode_return_list),np.std(episode_return_list)

    def run_evaluation_withstd(self, iteration):
        return self.run_n_episodes_withstd(self.num_eval_episode, iteration)
